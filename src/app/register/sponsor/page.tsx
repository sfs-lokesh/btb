...
export default function SponsorRegister() {
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            const res = await fetch('/api/register', {
                ...
                body: JSON.stringify({ ...data, role: 'Sponsor' }),
            });
    ...
    return (
        <div className="max-w-md mx-auto mt-10 p-6 bg-card border border-border rounded-lg shadow-md">
            <h1 className="text-2xl font-bold mb-6 text-foreground">Sponsor Registration</h1>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div>
                    <label className="block text-sm font-medium text-foreground">Company Name</label>
                    <input {...register('companyName', { required: true })} className="w-full bg-secondary border-input text-foreground p-2 rounded border" />
                </div>
                <div>
                ...
                <div>
                    <label className="block text-sm font-medium text-foreground">Sponsorship Tier</label>
                    <select {...register('tier')} className="w-full bg-secondary border-input text-foreground p-2 rounded border">
